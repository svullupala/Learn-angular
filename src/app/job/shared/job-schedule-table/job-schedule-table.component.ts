...
import {DefineEditScheduleModalComponent}
  from 'shared/components/define-edit-schedule/define-edit-schedule-modal/define-edit-schedule-modal.component';
...
import {TriggersModel} from 'job/shared/triggers.model';


@Component({
  selector: 'job-schedule-table',
  templateUrl: './job-schedule-table.component.html',
  styleUrls: ['./job-schedule-table.component.scss'],
  providers: [TriggerService]
})
export class JobScheduleTableComponent implements OnInit, OnDestroy, Sortable {

  static sessionPageSize: number = 10;

  @Output() jobsLoad = new EventEmitter<JobsModel>();
  @Output() edit = new EventEmitter<JobModel>();
  @ViewChild(DefineEditScheduleModalComponent) defineEditScheduleModal: DefineEditScheduleModalComponent;
  ...
  sessionPaginationInit(job: JobModel, idx: number): void {
    ...
    job.metadata['sessionModel'] =  new PaginateModel({id:  paginationSessionId,
      pageSize: JobScheduleTableComponent.sessionPageSize,
  ...
  isOnDemandJob(item): boolean {
    return (item.name).includes('onDemandRestore_');
  }
  ...
  private getJobTriggers(job: BaseModel): void {
    let me = this, observable = this.triggerService.getTriggers(job.getUrl('triggers'));
    me.mask();
    observable.takeUntil(me.subs).subscribe(
      dataset => {
        this.scheduleJobName = job.name;
        this.triggerId = job.triggerIds[0];
        this.defineEditScheduleModal.show(JsonConvert.deserializeObject(dataset.triggers[0].properties, ScheduleModel));
      },
      err => {},
      () => { me.unmask(); }
    );
  ...
  private onShowEditSchedule(item): void {
    this.getJobTriggers(item);
  }
  ...
  private onDetailClick(item: JobModel, autoRefreshMode?: boolean): void {
    ...
    if (!me.isLoading(item) && !me.isAnyChildLoading(undefined, item)
      ...
      observable = item.getDataset<JobSessionModel, JobSessionsModel>(JobSessionsModel, 'jobsessions', undefined,
        ...
        JobScheduleTableComponent.sessionPageSize);
      if (observable) {
        if (autoRefreshMode)
  ...
  private onDeleteClick(item: JobModel): void {
    ...
    me.confirm(item, function () {
      me.maskList = true;
      let provider = item.serviceId.split('.'), providerName = '';
      if (provider[provider.length - 1] === 'report')
      providerName = 'hypervisor';
      else
      providerName = provider[provider.length - 1];


      me.deleteSub = me.jobService.deleteJobsFromNode(providerName, item.name).subscribe(
        () => {
          me.maskList = false;
  ...
  private isDeletionAllowed(item: JobModel): boolean {
    let me = this;
    me.maskList = true;
    let provider = item.serviceId.split('.');

    if (((item.getLink('delete')) && (item.status === 'IDLE') && ((item.type === 'analyze') || (item.type === 'recovery'))))
    return true;
    else
    return false;
  }

  private isScheduleEditAllowed(item: JobModel): boolean {
    let me = this;
    me.maskList = true;
    let provider = item.serviceId.split('.');
    if ((item.triggerIds.length > 0) && (item.type !== 'protection'))
    return true;
    else
    return false;
  }
  ...
  private getSessionLocatePage(one?: boolean): void {
    let me = this, job = me.locateCtx.job, jobSessionId = me.locateCtx.jobSessionId,
      observable = job.getDataset<JobSessionModel, JobSessionsModel>(JobSessionsModel,
        ...
        [new SorterModel('start', 'DESC')],
        0, JobScheduleTableComponent.sessionPageSize);
    if (observable) {
      observable.takeUntil(me.subs).subscribe(
        dataset => {
          if (dataset) {
  ...
  private editSupported(item: JobModel): boolean {
    let ready = ['sql', 'oracle', 'db2', 'exch', 'mongo', 'hyperv', 'vmware', 'office365']; // TODO: add more for debug.
    return item && item.type === 'recovery' && ready.indexOf(item.subType) !== -1;
  }

  private isEditAllowed(item: JobModel): boolean {
    let supported = this.editSupported(item);
    return supported && item.hasLink('edit') && item.hasLink('policy') && item.hasLink('triggers');
  }

  private hasTrigger(item: JobModel): boolean {
    return item.triggerIds && item.triggerIds.length > 0;
  }

  private onEditClick(item: JobModel): void {
    let me = this;
    me.getPolicyInvokeEdit(item);
    if (me.hasTrigger(item))
      me.getTriggerInvokeEdit(item);
  }

  private editReady(item: JobModel): boolean {
    return item && item.spec && item.script && (item.trigger || !this.hasTrigger(item));
  }

  private getPolicyInvokeEdit(item: JobModel): void {
    let me = this, observable = item.getRecord<PolicyModel>(PolicyModel, 'policy');
    if (observable) {
      item.spec = undefined;
      ...
      observable.takeUntil(me.subs).subscribe(
        record => {
          if (record) {
            item.spec = record.spec;
            item.script = record.script;

            if (me.editReady(item))
              me.edit.emit(item);
          }
        },
        err => me.handleError(err)
  ...
  private getTriggerInvokeEdit(item: JobModel): void {
    let me = this, observable = item.getDataset<TriggerModel,
      TriggersModel>(TriggersModel, 'triggers');
    if (observable) {
      item.trigger = undefined;
      observable.takeUntil(me.subs).subscribe(
        dataset => {
          if (dataset && dataset.records && dataset.records.length > 0) {
            item.trigger = dataset.records[0].properties;
            if (me.editReady(item))
              me.edit.emit(item);
          }
        },
