...
@Component({
  selector: 'snapshot-restore-sql-source',
  templateUrl: './snapshot-restore-sql-source.component.html',
  styleUrls: ['../snapshot-restore-sql.scss']
})
export class SnapshotRestoreSqlSourceComponent extends WizardPage<SnapshotRestoreSqlModel> implements OnInit,
  OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Initialize views.
    me.views.push(
      new NvPairModel('', 'applicationview'),
      new NvPairModel('', 'databasegroupview'));
  ...
  startSearch(searchPattern: string): void {
    ...
    me.restoreTable.dbSearch(me.applicationType, 'recovery', {name: me.namePattern || ''},
      me.filters, me.view.value === me.views[1].value);
  }
  ...
  setSqlFilters(): void {
    ...
    if (dbGroupFilterIdx !== -1 && me.view) {
      me.filters[dbGroupFilterIdx].op = me.view.value === me.views[1].value ? 'EXISTS' : 'NOT EXISTS';
    }
  ...
  onBreadcrumbClick(item: any, event?: any): void {
    ...
    if (me.restoreTable) {
      ...
      // me.restoreTable.setFilters(me.filters);
      if (me.view.value === me.views[1].value) {
        dbView = true;
      }
  ...
  canNavigateBreadCrumb(item: ApplicationRestoreItem): boolean {
    if (item && item.resource) {
      return item.resource.hasLink(this.views[0].value)
        || item.resource.hasLink(this.views[1].value);
    }
    return true;
  }
  ...
  private isSqlAag(): boolean {
    return this.view && this.view.value === this.views[1].value;
  }
  ...
  private isStandalone(): boolean {
    ...
    //   && this.view && this.view.value === me.views[0].value;
    return this.view && this.view.value === this.views[0].value;
  }
  ...
  private isDbGroupView(): boolean {
    ...
    return (model.view.value === me.views[1].value);
  }
