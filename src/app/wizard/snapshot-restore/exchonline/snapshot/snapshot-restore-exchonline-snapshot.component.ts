...
import { AlertType, ErrorHandlerComponent } from 'app/shared/components';
...
@Component({
  selector: 'snapshot-restore-exchonline-snapshot',
  templateUrl: './snapshot-restore-exchonline-snapshot.component.html',
  styleUrls: ['../snapshot-restore-exchonline.scss']
})
export class SnapshotRestoreExchOnlineSnapshotComponent extends WizardPage<SnapshotRestoreExchOnlineModel>
  implements OnInit {
  ...
  private textPitEligibility: string;
  private textPitEligibilityWithContinue: string;
  private mask: boolean = false;
  private checkPitEligibilityPending: boolean = false;
  ...
  ngOnInit(): void {
    ...
    me.translate.get([
      ...
      'wizard.job.textPitEligibility',
      'wizard.job.textPitEligibilityWithContinue'
    ]).takeUntil(me.subs)
      .subscribe((resource: Object) => {
        me.infoTitle = resource['common.infoTitle'];
        me.textConfirm = resource['common.textConfirm'];
        me.textPitEligibility = resource['wizard.job.textPitEligibility'];
        me.textPitEligibilityWithContinue = resource['wizard.job.textPitEligibilityWithContinue'];
  ...
  public onActive(param: WizardPageEventParam, firstTime?: boolean): void {
    let me = this, checkPitEligibilityPending: boolean = false;
    if (!param.back) {
      me.updateRestoreTable();
      if (this.selectedRestoreSourceItem.value !== '' && !me.singleSourceOnDemand) {
        this.fillLatestSnapshots();
      }
      checkPitEligibilityPending = true;
    }
    if (me.editMode && firstTime) {
      me.checkPitEligibilityPending = checkPitEligibilityPending;
      me.populateOptions();
    } else {
      me.checkPitEligibilityPending = false;
      if (checkPitEligibilityPending && me.needCheckPitEligibility())
        me.checkPitEligibility(me.selectedRestoreType, true);
    }
  }
  ...
  private needCheckPitEligibility(selectedRestoreType?: NvPairModel): boolean {
    let me = this, checkCurrent = !selectedRestoreType, item = selectedRestoreType;
    return checkCurrent ? (me.selectedRestoreType && me.selectedRestoreType.value === 'pointintime') :
      (me.selectedRestoreType.value !== item.value && item.value === 'pointintime');
  }

  private excludeIneligibleSelections(): void {
    let me = this, model = me.model,
      list = me.getRestoreList() || [],
      target = list.filter(function (item) {
        return item.resource && item.resource.isLogBackupEligible();
      });
    model.source = [...target];
    me.updateRestoreTable();
    me.notify<ApplicationRestoreItem[]>('sourceEligibility', model.source);
  ...
  private checkPitEligibility(selectedRestoreType: NvPairModel, checkCurrent?: boolean): void {
    let me = this, handler, list = me.getRestoreList() || [],
      target = list.find(function (item) {
        return item.resource && !item.resource.isLogBackupEligible();
      });
    if (target) {
      handler = function () {
        me.excludeIneligibleSelections();
        if (!checkCurrent)
          me.handleSelectRestoreType(selectedRestoreType);
      };
      if (!checkCurrent || me.editMode)
        me.confirm(me.textPitEligibilityWithContinue, handler);
      else {
        me.confirm(me.textPitEligibility, () => {
          me.handleSelectRestoreType(me.restoreTypes[0]);
        }, () => {
          me.excludeIneligibleSelections();
        });
      }
  ...
  private onSelectRestoreType(item: NvPairModel): void {
    let me = this;
    if (me.needCheckPitEligibility(item))
      me.checkPitEligibility(item);
    else
      me.handleSelectRestoreType(item);
  }

  private handleSelectRestoreType(item: NvPairModel): void {
  ...
  private countOfLoadResources(): number {
  ...
  private populateOptions(): void {
    ...
    if (isRecurring || isPointInTime) {
      ...
      if (isPointInTime) {
        invokeCount = me.countOfLoadResources();
        me.loadResources(() => {
          invokeCount--;
          if (!invokeCount && me.checkPitEligibilityPending) {
            me.checkPitEligibilityPending = false;
            if (me.needCheckPitEligibility())
              me.checkPitEligibility(me.selectedRestoreType, true);
          }
        });
      }
