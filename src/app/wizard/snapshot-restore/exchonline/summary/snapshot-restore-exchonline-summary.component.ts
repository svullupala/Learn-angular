...
@Component({
  selector: 'snapshot-restore-exch-summary',
  templateUrl: './snapshot-restore-exchonline-summary.component.html',
  styleUrls: ['../snapshot-restore-exchonline.scss']
})
export class SnapshotRestoreExchOnlineSummaryComponent extends WizardPage<SnapshotRestoreExchOnlineModel> implements OnInit,
  OnDestroy {
  ...
  get summaryEntries(): SummaryEntry[] {
    ...
    return [
      ...
      { title: 'wizard.job.textJobOptionsSettings', content: me._summaryOptions },
  ...
  ngOnInit(): void {
    ...
    me.loadVersion();
    me.loadDestination();
    me.model.metadata = me.policy.spec.subpolicy[0].destination['metadata'];
  }
  ...
  get targetPath(): any {
    let subPolicy = this.subPolicy;
    return subPolicy.destination['targetPath'];
  }

  get destinationData(): any {
    let subPolicy = this.subPolicy;
    return subPolicy.destination['targetLocation'];
  }

  get tenantProviderIDSelected(): any {
    let subPolicy = this.subPolicy;
    if (subPolicy.destination['metadata'] && subPolicy.destination['metadata']['tenantProviderIDSelected']
      && subPolicy.destination['metadata']['tenantProviderIDSelected']['name']) {
      return subPolicy.destination['metadata']['tenantProviderIDSelected']['name'];
    }
    return undefined;
  }

  get selectedUsername(): any {
    let subPolicy = this.subPolicy;
    if (subPolicy.destination['metadata'] && subPolicy.destination['metadata']['user']
      && subPolicy.destination['metadata']['user']['username']) {
      return subPolicy.destination['metadata']['user']['username'];
    }
    return undefined;
  }
  ...
  get hasAllowsessoverwrite(): boolean {
    return !this.originalLocation && this.runType === 'test';
  }
