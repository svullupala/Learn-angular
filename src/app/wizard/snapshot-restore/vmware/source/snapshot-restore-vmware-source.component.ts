...
@Component({
  selector: 'snapshot-restore-vmware-source',
  templateUrl: './snapshot-restore-vmware-source.component.html',
  styleUrls: ['../snapshot-restore-vmware.scss']
})
export class SnapshotRestoreVmwareSourceComponent extends WizardPage<SnapshotRestoreVmwareModel> implements OnInit,
  OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Initialize views.
    me.views.push(
      new NvPairModel('', 'vmview'),
      new NvPairModel('', 'vms'),
      new NvPairModel('', 'tagview'),
      new NvPairModel('', 'hostview'));
    me.view = me.views[0];
    ...
    me.transSub = me.translate.get([
      ...
      .subscribe((resource: Object) => {
        ...
        me.views[0].name = resource['hypervisor.textVmAndTemplates'];
        me.views[1].name = resource['hypervisor.textVMView'];
        me.views[2].name = resource['hypervisor.textVMCategoryTagView'];
        me.views[3].name = resource['hypervisor.textHostsAndClustersView'];
      });
  }
  ...
  private findView(view: string): NvPairModel {
