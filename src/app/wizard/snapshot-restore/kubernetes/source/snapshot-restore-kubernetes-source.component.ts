...
@Component({
  selector: 'snapshot-restore-kubernetes-source',
  templateUrl: './snapshot-restore-kubernetes-source.component.html',
  styleUrls: ['../snapshot-restore-kubernetes.scss']
})
export class SnapshotRestoreKubernetesSourceComponent extends WizardPage<SnapshotRestoreKubernetesModel> implements OnInit,
  OnDestroy {
  ...
  ngOnInit(): void {
    ...
    me.views.push(
      ...
      new NvPairModel('', InstancesModel.LABEL_VIEW));
    ...
    me.translate.get([
      ...
      .subscribe((resource: Object) => {
        ...
        me.views[0].name = resource['application.textNamespace'];
        me.views[1].name = resource['application.textLabel'];
  ...
  canNavigateBreadCrumb(item: ApplicationRestoreItem): boolean {
    if (item && item.resource) {
      return item.resource.hasLink(InstancesModel.APPLICATION_VIEW)
        || item.resource.hasLink(InstancesModel.LABEL_VIEW);
    }
    return true;
  }
