...
@Component({
  selector: 'snapshot-restore-db2-options',
  templateUrl: './snapshot-restore-db2-options.component.html',
  styleUrls: ['../snapshot-restore-db2.scss']
})
export class SnapshotRestoreDb2OptionsComponent extends WizardPage<SnapshotRestoreDb2Model>
  implements OnInit {
  ...
  get hasAllowsessoverwrite(): boolean {
    return !this.model.originalLocation && this.model.runType === 'test';
  }
  ...
  private validAppOption(appOpt: any): boolean {
    return (appOpt.maxParallelStreams === undefined ||
      ...
      (appOpt.databaseMemory === undefined ||
        Number.isInteger(appOpt.databaseMemory) && appOpt.databaseMemory >= 0);
  }

  private initOptionForm(): void {
    let me = this,
      originalLocation = me.model.originalLocation,
    ...
    if (mode === ApplicationRestoreService.IA_VAL && type === ApplicationRestoreService.IA_VAL) {
      ...
      me.optionForm.hideElement('allowsessoverwrite');
    } else {
      if (!originalLocation && mode === 'test') {
        me.optionForm.showElement('allowsessoverwrite');
      } else {
        me.optionForm.hideElement('allowsessoverwrite');
      }
    }
    ...
    if (!originalLocation && mode === 'test') {
      me.optionForm.enableFormControl('allowsessoverwrite');
    } else {
