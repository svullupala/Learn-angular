...
@Component({
  selector: 'snapshot-restore-db2-destination',
  templateUrl: './snapshot-restore-db2-destination.component.html',
  styleUrls: ['../snapshot-restore-db2.scss']
})
export class SnapshotRestoreDb2DestinationComponent extends WizardPage<SnapshotRestoreDb2Model> implements OnInit {
  ...
  private onOriginalClick(value: string): void {
    this.model.runType = 'production';
    this.model.originalLocation = true;
    this.model.mappingsValue = this.model.defaultMappingsValueForProdOL;
    this.targetLocation = value;
  }

  private onAlternateLocationClick(value: string): void {
    this.targetLocation = value;
    this.model.originalLocation = false;
    this.applicationRestoreService.filterInstances(this.getRestoreList());
  }
