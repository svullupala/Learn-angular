...
import {SnapshotRestoreOracleSourceComponent} from './source/snapshot-restore-oracle-source.component';
import {SnapshotRestoreOracleSnapshotComponent} from './snapshot/snapshot-restore-oracle-snapshot.component';
import {SnapshotRestoreOracleDestinationComponent} from './destination/snapshot-restore-oracle-destination.component';
import {SnapshotRestoreOracleRunTypeComponent} from './run-type/snapshot-restore-oracle-run-type.component';
import {
  SnapshotRestoreOracleOptionsComponent
} from './options/snapshot-restore-oracle-options.component';
import {WIZARD_CATEGORY_SNAPSHOT_RESTORE} from '../snapshot-restore-wizard.model';
import {SnapshotRestoreOracleScriptsComponent} from './scripts/snapshot-restore-oracle-scripts.component';
import {SnapshotRestoreOracleScheduleComponent} from './schedule/snapshot-restore-oracle-schedule.component';
...
import {SnapshotRestoreOracleSummaryComponent} from './summary/snapshot-restore-oracle-summary.component';

export type SnapshotRestoreOraclePageComponent = SnapshotRestoreOracleSummaryComponent
...
@Injectable()
export class SnapshotRestoreOracleRegistry extends WizardRegistry {
  ...
  getModelClazz(category: WizardCategory): { new(): SnapshotRestoreOracleModel } {
    return SnapshotRestoreOracleModel;
  }

  getPageComponentFactory(category: WizardCategory,
                          pageIdx: number,
                          pageKey: string): ComponentFactory<SnapshotRestoreOraclePageComponent> {
    let me = this, factory: ComponentFactory<SnapshotRestoreOraclePageComponent>;
    switch (pageKey) {
      case 'summary':
        factory = me.resolver.resolveComponentFactory<SnapshotRestoreOracleSummaryComponent>(
          SnapshotRestoreOracleSummaryComponent);
        break;
      case 'source':
        factory = me.resolver.resolveComponentFactory<SnapshotRestoreOracleSourceComponent>(
          SnapshotRestoreOracleSourceComponent);
        break;
      case 'snapshot':
        factory = me.resolver.resolveComponentFactory<SnapshotRestoreOracleSnapshotComponent>(
          SnapshotRestoreOracleSnapshotComponent);
        break;
      case 'run-type':
        factory = me.resolver.resolveComponentFactory<SnapshotRestoreOracleRunTypeComponent>(
          SnapshotRestoreOracleRunTypeComponent);
        break;
      case 'destination':
        factory = me.resolver.resolveComponentFactory<SnapshotRestoreOracleDestinationComponent>(
          SnapshotRestoreOracleDestinationComponent);
        break;
      case 'options':
        factory = me.resolver.resolveComponentFactory<SnapshotRestoreOracleOptionsComponent>(
          SnapshotRestoreOracleOptionsComponent);
        break;
      case 'scripts':
        factory = me.resolver.resolveComponentFactory<SnapshotRestoreOracleScriptsComponent>(
          SnapshotRestoreOracleScriptsComponent);
        break;
      case 'schedule':
        factory = me.resolver.resolveComponentFactory<SnapshotRestoreOracleScheduleComponent>(
          SnapshotRestoreOracleScheduleComponent);
        break;
      default:
        break;
  ...
  buildPolicy(model: SnapshotRestoreOracleModel): void {
    let me = this,
      policy = model.policy,
  ...
  applyPolicy(model: SnapshotRestoreOracleModel): Observable<any> {
  ...
  playbackPolicy(model: SnapshotRestoreOracleModel): void {
    let me = this,
      policy = model.policy,
  ...
  pickEditMode(model: JobModel): SnapshotRestoreOracleModel {
    let me = this,
      result: SnapshotRestoreOracleModel;
    super.pickEditMode(model);
    result = new SnapshotRestoreOracleModel();
