...
@Component({
  selector: 'resource-group-database-selector',
  templateUrl: './resource-group-database-selector.component.html',
  styleUrls: ['../../resource-groups.scss'],
  providers: [ApplicationService]
})

export class ResourceGroupDatabaseSelectorComponent implements OnInit, OnDestroy {
  ...
  private resourceType: string = ResourceGroupsService.DATABASE_TYPE;
  ...
  private textAppMap = { all: '', sql: '', oracle: '', db2: '', alwayson: '', mongo: '', serverSql: '', exchange: '', exch: '', office365: ''};
  ...
  constructor(private rest: RestService,
    ...
    me.paginateConfig.id = `resource-group-database-table-pagination-${(new Date()).valueOf()}`;
  }
  ...
  ngOnInit(): void {
    ...
    me.translate.get([
      ...
      'application.textSqlFailover',
      'application.textDb2',
      'application.textSqlAlwayson',
      ...
      me.textAppMap = {
         ...
         sql: resource['application.textSql'],
         oracle: resource['application.textOracle'],
         db2: resource['application.textDb2'],
         alwayson: resource['application.textSql'],
         mongo: resource['application.textMongoDb'],
         serverSql: resource['application.textSql'],
         exchange: resource['application.textExchange'],
         exch: resource['application.textExchange'],
         office365: resource['application.textExchangeOnline'],
      };
  ...
  private isExch(): boolean {
    return this.applicationType === 'exch';
  }

  private isViewSupported(): boolean {
    return this.isSql() || this.isExch();
  }
  ...
  private resetApplicationType(selApplicationType: string) {
    ...
    switch (selApplicationType) {
      case 'exch':
        me.applicationTypeText = me.textAppMap.exchange;
        return;
      case 'office365':
        me.applicationTypeText = me.textAppMap.office365;
        return;
      case 'db2':
        me.applicationTypeText = me.textAppMap.db2;
        return;
      case 'mongo':
        me.applicationTypeText = me.textAppMap.mongo;
        return;
      case 'oracle':
        me.applicationTypeText = me.textAppMap.oracle;
        return;
      case 'sql':
        me.applicationTypeText = me.textAppMap.sql;
        return;
      case 'alwayson':
  ...
  private loadData(isAlwaysOn: boolean = false): void {
    ...
    if ((me.isViewSupported() && me.typeView === 'databasegroupview') || alwaysOn) {
      me.view = InstancesModel.DATABASE_GROUP_VIEW;
      alwaysOn = true;
    } else {
  ...
  private navigate(item: BaseApplicationModel): void {
    ...
    if (me.isViewSupported() && me.typeView === 'databasegroupview') {
      me.view = InstancesModel.DATABASE_GROUP_VIEW;
    } else {
      me.view = InstancesModel.APPLICATION_VIEW;
    }
