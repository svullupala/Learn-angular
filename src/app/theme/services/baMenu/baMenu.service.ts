...
export class BaMenuService {
  ...
  public static subMenuId(item: any): string {
    let paths = item.route && Array.isArray(item.route.paths) ? item.route.paths : [],
      keyStr = 'ba-menu-item-' + paths.join('/') + '-' + item.title || '';
    return MD5.encode(keyStr);
  }
  ...
  public selectMenuItem(menuItems:any[], parent?: any[]):any[] {
    ...
    menuItems.forEach((item) => {
      ...
      if (item.children && item.children.length > 0) {
        item.children = this.selectMenuItem(item.children, parent ? parent.concat([item]) : [item]);
      }
      items.push(item);
    });
    return items;
