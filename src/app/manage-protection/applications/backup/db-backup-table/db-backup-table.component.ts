...
import { BaseApplicationModel, PartitionModel } from '../../shared/base-application-model.model';


@Component({
  selector: 'db-backup-table',
  templateUrl: 'db-backup-table.component.html',
  styleUrls: ['db-backup-table.component.scss'],
  providers: [
    {provide: SelectorService, useFactory: selectorFactory, deps: [SelectorType]},
    {provide: SelectorType, useValue: SelectorType.MULTIHIER}
  ]
})
export class DbBackupTableComponent implements OnInit, OnDestroy {
  ...
  canNavigate(item: BaseApplicationModel): boolean {
    if (item && (item.resourceType === 'applicationinstance' && item.type !== 'saphana' ||
                item.resourceType === 'databasegroup' || item.resourceType === 'folder' || item.resourceType === 'label')) {
      return true;
    }
    return false;
  }
  ...
  private hideLogBackupCol(): boolean {
    return (this.applicationService.isDbSearch || this.applicationService.isDbLevel) && this.applicationType !== 'office365';
  }
