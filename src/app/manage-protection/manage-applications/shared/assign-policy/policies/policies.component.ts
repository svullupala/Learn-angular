...
import { ApplicationsAssignPolicyService } from '../applications-assign-policy.service';
...
@Component({
  selector: 'applications-assign-policy-policies',
...
export class PoliciesComponent implements OnInit {
  ...
  constructor(
    ...
    private applicationsBackupService: ApplicationBackupService,
  ...
  get formattedTextAssignToProtectResourcesInfo(): string {
    return SharedService.formatString(this.textAssignToProtectResourcesInfo, this.assignPolicyTo.name);
  }
  ...
  onAssignPolicies(): void {
    this.masked = true;
    this.applicationsBackupService
      .applyPolicies([this.assignPolicyTo], this.selector.selection(), this.applicationType)
      .subscribe(
        () => {
          this.masked = false;
